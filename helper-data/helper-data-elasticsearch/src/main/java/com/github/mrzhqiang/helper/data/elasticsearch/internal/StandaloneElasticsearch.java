package com.github.mrzhqiang.helper.data.elasticsearch.internal;

import com.github.mrzhqiang.helper.data.elasticsearch.Elasticsearch;
 ...
 * @author qiang.zhang
 */
@Slf4j(topic = "helper.data.elasticsearch")
@Singleton
public final class StandaloneElasticsearch implements Elasticsearch {
    ...
    public StandaloneElasticsearch() {
        ...
        restClient = Datas.get(() -> RestClient.builder(new HttpHost(host, port, scheme)).build());
    ...
    @Override
    public void execute(Consumer<RestClient> consumer) {
        if (!enabled || Objects.isNull(restClient)) {
        ...
        Datas.accept(restClient, consumer);
    }

    @Override
    public <T> Optional<T> find(Function<RestClient, T> function) {
        if (!enabled || Objects.isNull(restClient)) {
            log.warn("Elasticsearch is disabled.");
        ...
        return Optional.ofNullable(Datas.apply(restClient, function));
    }
}
